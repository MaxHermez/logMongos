...
func parseURI(shards string, replica string, db string) string {
	uri := "mongodb://" + shards + "/" + db + "ssl=true&replicaSet=" + replica + "&authSource=%24external&authMechanism=MONGODB-X509&retryWrites=true&w=majority&tlsCaFile=/etc/ssl/certs/mongocert.pem"
	return uri
}
